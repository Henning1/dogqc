...
__global__ void krnl_customer4(
    ...
    while(!(flushPipeline)) {
        ...
        // -------- multiprobe multi broadcast (opId: 5) --------
        int matchEnd5 = 0;
        int matchEndBuf5 = 0;
        int matchOffset5 = 0;
        int matchOffsetBuf5 = 0;
        int probeActive5 = active;
        int att15_ccustkey_bcbuf5;
        str_t att16_cname_bcbuf5;
        str_t att17_caddress_bcbuf5;
        int att18_cnationk_bcbuf5;
        str_t att19_cphone_bcbuf5;
        float att20_cacctbal_bcbuf5;
        str_t att22_ccomment_bcbuf5;
        uint64_t hash5 = 0;
        if(probeActive5) {
            hash5 = 0;
            if(active) {
            ...
            probeActive5 = hashProbeMulti ( jht5, 150000, hash5, matchOffsetBuf5, matchEndBuf5);
        }
        unsigned activeProbes5 = __ballot_sync(ALL_LANES,probeActive5);
        int num5 = 0;
        num5 = (matchEndBuf5 - matchOffsetBuf5);
        unsigned wideProbes5 = __ballot_sync(ALL_LANES,(num5 >= 32));
        att15_ccustkey_bcbuf5 = att15_ccustkey;
        att16_cname_bcbuf5 = att16_cname;
        att17_caddress_bcbuf5 = att17_caddress;
        att18_cnationk_bcbuf5 = att18_cnationk;
        att19_cphone_bcbuf5 = att19_cphone;
        att20_cacctbal_bcbuf5 = att20_cacctbal;
        att22_ccomment_bcbuf5 = att22_ccomment;
        while((activeProbes5 > 0)) {
            unsigned tupleLane;
            unsigned broadcastLane;
            int numFilled = 0;
            int num = 0;
            while(((numFilled < 32) && activeProbes5)) {
                if((wideProbes5 > 0)) {
                    tupleLane = (__ffs(wideProbes5) - 1);
                    wideProbes5 -= (1 << tupleLane);
                }
                else {
                    tupleLane = (__ffs(activeProbes5) - 1);
                }
                num = __shfl_sync(ALL_LANES,num5,tupleLane);
                if((numFilled && ((numFilled + num) > 32))) {
                    break;
                }
                if((warplane >= numFilled)) {
                    broadcastLane = tupleLane;
                    matchOffset5 = (warplane - numFilled);
                }
                numFilled += num;
                activeProbes5 -= (1 << tupleLane);
            }
            matchOffset5 += __shfl_sync(ALL_LANES,matchOffsetBuf5,broadcastLane);
            matchEnd5 = __shfl_sync(ALL_LANES,matchEndBuf5,broadcastLane);
            att15_ccustkey = __shfl_sync(ALL_LANES,att15_ccustkey_bcbuf5,broadcastLane);
            att16_cname = __shfl_sync(ALL_LANES,att16_cname_bcbuf5,broadcastLane);
            att17_caddress = __shfl_sync(ALL_LANES,att17_caddress_bcbuf5,broadcastLane);
            att18_cnationk = __shfl_sync(ALL_LANES,att18_cnationk_bcbuf5,broadcastLane);
            att19_cphone = __shfl_sync(ALL_LANES,att19_cphone_bcbuf5,broadcastLane);
            att20_cacctbal = __shfl_sync(ALL_LANES,att20_cacctbal_bcbuf5,broadcastLane);
            att22_ccomment = __shfl_sync(ALL_LANES,att22_ccomment_bcbuf5,broadcastLane);
            probeActive5 = (matchOffset5 < matchEnd5);
            while(__any_sync(ALL_LANES,probeActive5)) {
                active = probeActive5;
                active = 0;
                jpayl5 payl;
                if(probeActive5) {
...
int main() {
    ...
    std::clock_t start_totalKernelTime86 = std::clock();
    std::clock_t start_krnl_nation187 = std::clock();
    {
        int gridsize=920;
        int blocksize=128;
    ...
    cudaDeviceSynchronize();
    std::clock_t stop_krnl_nation187 = std::clock();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
    ...
    std::clock_t start_krnl_orders288 = std::clock();
    {
        int gridsize=920;
        int blocksize=128;
    ...
    cudaDeviceSynchronize();
    std::clock_t stop_krnl_orders288 = std::clock();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
    ...
    std::clock_t start_scanMultiHT89 = std::clock();
    {
        int gridsize=920;
        int blocksize=128;
    ...
    cudaDeviceSynchronize();
    std::clock_t stop_scanMultiHT89 = std::clock();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
    ...
    std::clock_t start_krnl_orders2_ins90 = std::clock();
    {
        int gridsize=920;
        int blocksize=128;
    ...
    cudaDeviceSynchronize();
    std::clock_t stop_krnl_orders2_ins90 = std::clock();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
    ...
    std::clock_t start_krnl_customer491 = std::clock();
    {
        int gridsize=920;
        int blocksize=128;
    ...
    cudaDeviceSynchronize();
    std::clock_t stop_krnl_customer491 = std::clock();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
    ...
    std::clock_t start_krnl_lineitem792 = std::clock();
    {
        int gridsize=920;
        int blocksize=128;
    ...
    cudaDeviceSynchronize();
    std::clock_t stop_krnl_lineitem792 = std::clock();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
    ...
    std::clock_t start_krnl_aggregation1193 = std::clock();
    {
        int gridsize=920;
        int blocksize=128;
    ...
    cudaDeviceSynchronize();
    std::clock_t stop_krnl_aggregation1193 = std::clock();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
    ...
    std::clock_t stop_totalKernelTime86 = std::clock();
    ...
    std::clock_t start_finish94 = std::clock();
    printf("\nResult: %i tuples\n", nout_result);
    ...
    printf("\n");
    std::clock_t stop_finish94 = std::clock();

    printf("<timing>\n");
    printf ( "%32s: %6.1f ms\n", "krnl_nation1", (stop_krnl_nation187 - start_krnl_nation187) / (double) (CLOCKS_PER_SEC / 1000) );
    printf ( "%32s: %6.1f ms\n", "krnl_orders2", (stop_krnl_orders288 - start_krnl_orders288) / (double) (CLOCKS_PER_SEC / 1000) );
    printf ( "%32s: %6.1f ms\n", "scanMultiHT", (stop_scanMultiHT89 - start_scanMultiHT89) / (double) (CLOCKS_PER_SEC / 1000) );
    printf ( "%32s: %6.1f ms\n", "krnl_orders2_ins", (stop_krnl_orders2_ins90 - start_krnl_orders2_ins90) / (double) (CLOCKS_PER_SEC / 1000) );
    printf ( "%32s: %6.1f ms\n", "krnl_customer4", (stop_krnl_customer491 - start_krnl_customer491) / (double) (CLOCKS_PER_SEC / 1000) );
    printf ( "%32s: %6.1f ms\n", "krnl_lineitem7", (stop_krnl_lineitem792 - start_krnl_lineitem792) / (double) (CLOCKS_PER_SEC / 1000) );
    printf ( "%32s: %6.1f ms\n", "krnl_aggregation11", (stop_krnl_aggregation1193 - start_krnl_aggregation1193) / (double) (CLOCKS_PER_SEC / 1000) );
    printf ( "%32s: %6.1f ms\n", "finish", (stop_finish94 - start_finish94) / (double) (CLOCKS_PER_SEC / 1000) );
    printf ( "%32s: %6.1f ms\n", "totalKernelTime", (stop_totalKernelTime86 - start_totalKernelTime86) / (double) (CLOCKS_PER_SEC / 1000) );
    printf("</timing>\n");
}
